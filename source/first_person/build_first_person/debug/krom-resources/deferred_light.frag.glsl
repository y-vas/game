...
float shadowTestCascade(sampler2DShadow shadowMap_1, vec3 eye_1, vec3 p, float shadowsBias_1)
{
    ...
    {
        _508 = nextSplit - (casData[16][casi - 1]);
    }
...
void main()
{
    ...
    vec3 voxpos = p / vec3(16.0, 16.0, 8.0);
    envl *= (1.0 - traceAO(voxpos, n, voxels));
    fragColor = vec4(envl.x, envl.y, envl.z, fragColor.w);
    vec3 _1069 = fragColor.xyz * textureLod(ssaotex, texCoord, 0.0).x;
    fragColor = vec4(_1069.x, _1069.y, _1069.z, fragColor.w);
    if (g0.w == 1.0)
    {
        vec3 _1081 = fragColor.xyz + g1.xyz;
        fragColor = vec4(_1081.x, _1081.y, _1081.z, fragColor.w);
        albedo = vec3(0.0);
    }
    ...
    vec3 _1138 = fragColor.xyz + ((sdirect * svisibility) * sunCol);
    fragColor = vec4(_1138.x, _1138.y, _1138.z, fragColor.w);
    fragColor.w = 1.0;
}
