import sys
from bge import logic as lg, render, events
...
def lib(*params):
    ...
    root = sys.path[0] + '/source'

    libs = {
        "hud" : root + '/Display/hud.blend',
        "move": root + '/data/move.blend'
    }

    scen = lg.getCurrentScene()
    names = [x for x in scen.objects]

    for lib in params:
        lg.LibLoad(libs[lib],'Scene')

    nnam = []

    for obj in scen.objects:
        if obj in names: continue
        obj.visible = False
        nnam.append(obj)
